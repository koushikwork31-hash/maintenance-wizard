...
class MaintenanceOrchestrator:
    ...
    def _run_mock_workflow(self, query: str) -> Dict[str, Any]:
        ...
        return {
            "analysis": """### 🛠️ Lead Engineer's Autonomous Assessment (Multi-Agent Synthesis)

#### **Equipment Status: BF-01**
- **Temperature**: 18.3°C (↑ 140% from baseline)
- **Flow Rate**: 32.1 m³/h (↓ 31% from optimal)
- **Health Score**: 52/100 (Critical)

#### **Root Cause Analysis**
1. **Telemetry Agent**: Detected severe flow restriction in Circuit 4, consistent with scale buildup.
2. **Reliability Agent**: Historical records (REC‑102, REC‑115) confirm identical symptoms preceded 2025 pipe fatigue failure.
3. **SOP Specialist**: Retrieving descaling procedure from Blast Furnace Cooling Manual, Section 7.2.

#### **Action Plan**
1. **Immediate**: Activate LOTO‑2026 lockout for Circuit 4.
2. **Prep**: Don full PPE (chemical‑resistant suit, goggles, gloves).
3. **Execution**: Apply approved descaling agent per SOP‑BF‑07.
4. **Validation**: Post‑descaling pressure test to confirm >95% flow restoration.

#### **Safety Validation**
✅ All mandatory protocols verified.
✅ LOTO procedure documented.
